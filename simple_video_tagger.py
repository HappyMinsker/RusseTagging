#!/usr/bin/env python3
"""
Simple Video Metadata Tagger
A lightweight version that only requires ffmpeg-python.
"""
...
class SimpleVideoTagger:
    """A simple class to handle video metadata tagging operations."""
    ...
    def write_metadata(self, metadata: Dict[str, str], output_path: Optional[str] = None) -> bool:
        """Write metadata to the video file using ffmpeg CLI for better MP4 support."""
    ...
    def remove_tag(self, key: str, output_path: Optional[str] = None) -> bool:
        ...
        if key in current_metadata:
            ...
            print(f"Tag '{key}' not found in metadata")
            return False
    ...
    def clear_all_metadata(self, output_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            print(f"✓ All metadata cleared from: {output_path or str(self.video_path)}")
            return True
    ...
    def display_metadata(self):
        ...
        if not metadata:
            print(f"No metadata found in: {self.video_path}")
            return
        
        print(f"\nMetadata for: {self.video_path}")
        print("="*50)
    ...
    def get_video_title_from_filename(self) -> str:
    ...
    def load_metadata_with_auto_title(self, metadata: Dict[str, str]) -> Dict[str, str]:
        """Load metadata and automatically set title from filename if not present."""
        # Create a copy to avoid modifying the original
        result_metadata = metadata.copy()
        
        # If title is not set or is empty, use filename
        if 'title' not in result_metadata or not result_metadata['title'].strip():
            result_metadata['title'] = self.get_video_title_from_filename()
...
Examples:
  ...
  python simple_video_tagger.py video.mp4 --batch metadata.json
    ...
    try:
        tagger = SimpleVideoTagger(args.video_file)
        ...
        if args.add:
            for key, value in args.add:
                ...
                if not success:
                    print(f"Failed to add tag: {key}")
        
        # Remove metadata tag
        if args.remove:
            print(f"Removing tag: {args.remove}")
            tagger.remove_tag(args.remove, args.output)
        
        # Clear all metadata
        if args.clear:
            print(f"Clearing all metadata...")
        ...
        if args.batch:
            try:
                ...
                print(f"Loading metadata from: {args.batch}")
                ...
                if success:
                    print(f"✓ Batch metadata applied successfully")
                else:
                    print(f"Failed to apply batch metadata")
                    
            except FileNotFoundError:
                print(f"JSON file not found: {args.batch}")
            except json.JSONDecodeError:
                print(f"Invalid JSON format in: {args.batch}")
